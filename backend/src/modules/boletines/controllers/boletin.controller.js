...
class BoletinController {
  constructor(servicio) {
    ...
    ["listarPublico", "obtenerAdministracion", "guardarColeccion",
      ...
      "listarCategoriasCorreo", "listarDestinatariosCorreo",
      "obtenerDestinatarioCorreo", "crearDestinatarioCorreo",
      "actualizarDestinatarioCorreo", "cambiarEstadoDestinatarioCorreo",
      "resolverSeleccionCorreo", "enviarBoletinCorreo",
      "listarEnviosCorreo", "obtenerDetalleEnvioCorreo",
      "obtenerEnvioBoletin"]
  ...
  async listarCategoriasCorreo(_req, res, next) {
    try { return respuestaExitosa(res, "Categorías de destinatarios obtenidas correctamente.", await this.servicio.listarCategoriasCorreo()); }
    catch (error) { return next(error); }
  }

  async listarDestinatariosCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Directorio de correos obtenido correctamente.", await this.servicio.listarDestinatariosCorreo(req.query)); }
    catch (error) { return next(error); }
  }

  async obtenerDestinatarioCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Destinatario obtenido correctamente.", await this.servicio.obtenerDestinatarioCorreo(req.params.idDestinatario)); }
    catch (error) { return next(error); }
  }

  async crearDestinatarioCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Destinatario creado correctamente.", await this.servicio.guardarDestinatarioCorreo(req.body, this.contexto(req)), 201); }
    catch (error) { return next(error); }
  }

  async actualizarDestinatarioCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Destinatario actualizado correctamente.", await this.servicio.guardarDestinatarioCorreo({ ...req.body, idDestinatario: req.params.idDestinatario }, this.contexto(req))); }
    catch (error) { return next(error); }
  }

  async cambiarEstadoDestinatarioCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Estado del destinatario actualizado correctamente.", await this.servicio.cambiarEstadoDestinatarioCorreo(req.params.idDestinatario, req.body.activo, this.contexto(req))); }
    catch (error) { return next(error); }
  }

  async resolverSeleccionCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Selección de destinatarios resuelta correctamente.", await this.servicio.resolverSeleccionCorreo(req.body)); }
    catch (error) { return next(error); }
  }

  async enviarBoletinCorreo(req, res, next) {
    try { return respuestaExitosa(res, "El envío del boletín fue procesado.", await this.servicio.enviarBoletinCorreo(req.body, this.contexto(req)), 201); }
    catch (error) { return next(error); }
  }

  async listarEnviosCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Historial de envíos obtenido correctamente.", await this.servicio.listarEnviosCorreo(req.query)); }
    catch (error) { return next(error); }
  }

  async obtenerDetalleEnvioCorreo(req, res, next) {
    try { return respuestaExitosa(res, "Detalle del envío obtenido correctamente.", await this.servicio.obtenerDetalleEnvioCorreo(req.params.idEnvio)); }
    catch (error) { return next(error); }
  }

  async obtenerEnvioBoletin(req, res, next) {
    try { return respuestaExitosa(res, "Estado de correo del boletín obtenido correctamente.", await this.servicio.obtenerEnvioBoletin(req.params.idElemento)); }
    catch (error) { return next(error); }
  }
}

module.exports = new BoletinController(contenidoServices["boletines"]);
